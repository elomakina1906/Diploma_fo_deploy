...
def about(request):
    ...
    if city != "":
        if month != "":
            ...
            if r != 0:
                rating = {'rating': r}
                return render(request, 'first/rating_city_month.html', rating)
            else:
        ...
        if month != "":
            ...
            return render(request, 'first/for_lists.html', rating)
        else:
...
def month_to_top_of_cities(month, data):
    ...
    for value in data.iloc:
        i += 1
        if value['дата'].split(' ')[3].startswith(month_we_need):
            city = value['город']
